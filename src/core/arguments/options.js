  ...
  async run(msg, arg, tag) {
    if(tag.literal) {
      ...
      if(!tag.options.includes(input))
        throw msg.locale.t("ARGUMENT_BAD_OPTIONS", tag);
    ...
    for(const option of tag.options) {
      ...
      if(!arg) throw msg.locale.t("ARGUMENT_OPTIONS_BAD_TYPE", option);
      try {
    ...
    throw msg.locale.t("ARGUMENT_OPTIONS_NONE_MATCH", tag);
  }
}
